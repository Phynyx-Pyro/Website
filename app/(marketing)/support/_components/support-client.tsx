        ...
        <AnimatedSection delay={100}>
          <form onSubmit={handleSubmit} className="rounded-2xl bg-white p-8 shadow-xl">
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="support-name" className="block text-[13px] font-medium text-ink mb-1.5">Name *</label>
                <input id="support-name" name="name" type="text" autoComplete="name" required value={form?.name ?? ''} onChange={(e) => update('name', e?.target?.value ?? '')} className="w-full rounded-lg border border-ink/15 bg-ivory px-4 py-3 text-[15px] text-ink placeholder:text-warm/50 focus:border-phoenix focus:ring-1 focus:ring-phoenix outline-none transition" placeholder="Your name" />
              </div>
              <div>
                <label htmlFor="support-email" className="block text-[13px] font-medium text-ink mb-1.5">Email *</label>
                <input id="support-email" name="email" type="email" autoComplete="email" required value={form?.email ?? ''} onChange={(e) => update('email', e?.target?.value ?? '')} className="w-full rounded-lg border border-ink/15 bg-ivory px-4 py-3 text-[15px] text-ink placeholder:text-warm/50 focus:border-phoenix focus:ring-1 focus:ring-phoenix outline-none transition" placeholder="you@example.com" />
              </div>
              <div>
                <label htmlFor="support-subject" className="block text-[13px] font-medium text-ink mb-1.5">Subject</label>
                <input id="support-subject" name="subject" type="text" value={form?.subject ?? ''} onChange={(e) => update('subject', e?.target?.value ?? '')} className="w-full rounded-lg border border-ink/15 bg-ivory px-4 py-3 text-[15px] text-ink placeholder:text-warm/50 focus:border-phoenix focus:ring-1 focus:ring-phoenix outline-none transition" placeholder="Account help, billing, technical issue..." />
              </div>
              <div>
                <label htmlFor="support-message" className="block text-[13px] font-medium text-ink mb-1.5">Message *</label>
                <textarea id="support-message" name="message" required value={form?.message ?? ''} onChange={(e) => update('message', e?.target?.value ?? '')} rows={5} className="w-full rounded-lg border border-ink/15 bg-ivory px-4 py-3 text-[15px] text-ink placeholder:text-warm/50 focus:border-phoenix focus:ring-1 focus:ring-phoenix outline-none transition resize-none" placeholder="Tell us how we can help..." />
              </div>
            </div>
