...
7. Before enabling the concierge's live channel, decide and test the bot's broad cancel/reschedule capability against existing appointments, or turn those capabilities off and route such requests to a person. Its current action editor permits both, but no internal appointment-pair test has established safe scope. Direct bot assignment in 002b can override channel/tag routing; merely adding a QA tag filter to an Email deployment is not a proven isolation boundary.
