...
test('assessment CTAs map visitor-facing placements to the deployed hyphenated contract', async () => {
  ...
  const mappedOrigins = new Set(
    [...componentSource.matchAll(/:\s*'([a-z0-9]+(?:-[a-z0-9]+)*)',/g)].map(
      ([, origin]) => origin,
    ),
  )
  assert.deepEqual(
  ...
  assert.match(componentSource, /href=\{`\/growth-assessment\?\$\{params\.toString\(\)\}`\}/)
  assert.match(componentSource, /data-cta-placement=\{livePlacement\}/)
  assert.doesNotMatch(componentSource, /growth-assessment\/start|funnel-events/)

  const quickFormSource = await readFile(
    new URL(
  ...
  assert.match(quickFormSource, /set\('cta', 'homepage-quick-form'\)/)
  assert.match(quickFormSource, /router\.push\(`\/growth-assessment\?/)
  assert.doesNotMatch(quickFormSource, /growth-assessment\/start|funnel-events/)
})
